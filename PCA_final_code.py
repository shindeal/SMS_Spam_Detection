# -*- coding: utf-8 -*-
"""
Created on Fri Nov 18 18:33:53 2022
...
sms_text_data_df["value"] = sms_text_data_df["value"].apply(preprocessing_cleantext) #clean_text,preprocessing_cleantext
...
cv=CountVectorizer() 
word_count_vector=cv.fit_transform(sms_text_data_df["value"])
word_count_vector.shape
#tfidf_transformer=TfidfTransformer() 
#tfidf_transformer.fit(word_count_vector)
#df_idf = pd.DataFrame(tfidf_transformer.idf_, index=cv.get_feature_names(),columns=["idf_weights"]) 
#df_idf.sort_values(by=['idf_weights']) 
count_vector=cv.transform(sms_text_data_df["value"]) 
...
pca=SparsePCA(n_components = 25)
x_train_pca=pca.fit_transform(count_vector.toarray())
print(x_train_pca.shape)


k_clusters = 2
model = KMeans(n_clusters=k_clusters, init='k-means++', n_init=10, max_iter=600, tol=0.000001,copy_x=True)
model.fit(x_train_pca)
clusters = model.predict(x_train_pca)
...
pickle.dump(pca, open('tfidf.pkl', 'wb'))
...
count_vector=cv.transform(sms_text_data_pred_df['value']) 

model = pickle.load(open('model.pkl', 'rb'))
tfidf = pickle.load(open('tfidf.pkl', 'rb'))

sms_text_unseen = tfidf.transform(count_vector.toarray())
